...
trivialSubgroup = setFromFoldable [mempty]
...
subgroup = foldl (flip extendSubgroup) trivialSubgroup <<< setToArray
...
-- Sets;purescript-sets is missing findMin
...
setFromFoldable :: forall f a. Ord a => Foldable f => f a -> Set a
setFromFoldable = Set <<< foldl (\m k -> Map.insert k unit m) Map.empty

setToArray :: forall a. Set a -> Array a
setToArray (Set m) = Array.fromFoldable (Map.keys m)

mapSet :: forall a b. Ord a => Ord b => (a -> b) -> Set a -> Set b
mapSet f = setFromFoldable <<< map f <<< setToArray

instance showSet :: Show a => Show (Set a) where
  show a =
    "{"
    <> String.joinWith ", " (map show (setToArray a))
    <> "}"
